...
class MainRecyclerAdapter(
    ...
    inner class MainViewHolder(itemView: View, viewPool: RecyclerView.RecycledViewPool?) : RecyclerView.ViewHolder(itemView) {
        fun bindTitle(title: String) {
            itemView.textTitle.text = title
        }

        fun bindMovies(movies: List<Movie>) {
            itemView.recyclerMovies.adapter = MoviesRecyclerAdapter(movies)
